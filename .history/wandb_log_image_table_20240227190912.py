...
run = wandb.init(project = "aipi549", name = "wandb_log_image_table"+ "_" + args.core + "_" + str(args.n_imgs))

for n in range(0, args.n_img, 10):
    if n == 0:
        n = 1
    ...
    artifact = wandb.Artifact(name="image_table", type="dataset", metadata = {"n_img":n, **hardware_info})
    ...
    run.log({"table size ": get_image_memory_size(data_dir)*n})
run.finish()